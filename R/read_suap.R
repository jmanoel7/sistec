...
#' @return A data frame.
#'
#' @details For Suap datasets follow these steps:
#'
...
read_suap <- function(path = "", start = NULL) {
  ...
  if (sum(names(suap) %in% vars) == 8) {
    ...
    stop(paste(
      "Not found:", paste(vars[!vars %in% names(suap)], collapse = ", ")
    ))
  }
...
suap_convert_beginning_date <- function(date) {
  ...
  year_semester <- paste0(year, ".", ifelse(month <= 6, 1, 2))
...
suap_cota <- function(cota) {
  dplyr::if_else(
    stringr::str_detect(cota, "Renda|Qualquer|Autodeclarados"),
    "COTISTA",
    "N\u00c3O COTISTA"
  )
}
